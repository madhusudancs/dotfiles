# Linux desktop -- layered on top of linux.nix for graphical hosts only.
#
# Everything here assumes a display stack: nixGL-wrapped GUI apps (which probe
# the host GPU at build time, hence `home-manager switch --impure`), GNOME/XDG
# desktop-entry registration, fontconfig, gnome-keyring, and the Bitwarden SSH
# agent socket. Headless hosts inherit linux.nix without this file.
